...
SVG
    ...
    sub group_start {
        ...
        if ( $attr->{href} ) {
            my @a_attr;
            push @a_attr, sprintf qq/xlink:href="%s"/, $attr->{href}
                if $attr->{href};
    ...
    sub filledRectangle {
        ...
        $self->{svg}
            .= qq/<rect x="$x1" y="$y1" width="$w" height="$h" fill="$fill" $extra \/>\n/;
    }

    sub stringTTF {
        my ($self, $color, $font, $size, $angle,
            $x,    $y,     $str,  $loc,  $extra
        ) = @_;
        $x = sprintf "%0.2f", $x;
        ...
        $extra = defined $extra ? $extra : "";
        $self->{svg}
            .= qq/<text text-anchor="$loc" x="$x" y="$y" font-size="$size" font-family="$font" fill="$color" $extra >$str<\/text>\n/;
    }
...
foreach (<>) {
    ...
    if ($stackreverse) {
        ...
        if ( $stack =~ /^(.*)\s+?(\d+(?:\.\d*)?)$/ ) {
            ...
            unshift @Data,
                join( ";", reverse split( ";", $stack ) )
                . " $samples $samples2";
        }
        else {
            unshift @Data,
                join( ";", reverse split( ";", $stack ) ) . " $samples";
        }
    }
    else {
        unshift @Data, $line;
...
if ( $timemax and $timemax < $time ) {
    warn
        ...
        if $timemax / $time
        > 0.02;    # only warn is significant (e.g., not rounding etc)
    undef $timemax;
}
$timemax ||= $time;
...
$im->stringTTF(
    ...
    "Search",
    "",
    'id="search" onmouseover="searchover()" onmouseout="searchout()" onclick="search_prompt()" style="opacity:0.1;cursor:pointer"'
);
$im->stringTTF(
    $black, $fonttype, $fontsize, 0.0,
    $imagewidth - $xpad - 100,
    $imageheight - ( $ypad2 / 2 ),
    " ", "", 'id="matched"'
);
...
while ( my ( $id, $node ) = each %Node ) {
    ...
    if ( $func eq "" and $depth == 0 ) {
        ...
        unless ( defined $delta ) {
            ...
            my $deltapct = sprintf "%.2f",
                ( ( 100 * $d ) / ( $timemax * $factor ) );
            $deltapct = $d > 0 ? "+$deltapct" : $deltapct;
            $info
                = "$escaped_func ($samples_txt $countname, $pct%; $deltapct%)";
        }
    }
