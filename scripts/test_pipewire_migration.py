...
class PipeWireMigrationTest(unittest.TestCase):
    ...
    def test_bluetooth_default_service_routes_existing_app_streams(self):
        ...
        self.assertIn('self.ops.call("set-sink-mute", name, str(int(sink_mute)))', script)
        self.assertIn('self.ops.call("set-source-mute", name, str(int(source_mute)))', script)
        self.assertNotIn("set-sink-input-mute", script)
        self.assertNotIn("set-source-output-mute", script)
