...
set -x FLAMEGRAPH_DIR /home/amos/git/FlameGraph
set -x UID (id -u)
...
# prepend path
set -U fish_user_paths $HOME/scripts $FZF_HOME/bin $GOPATH/bin $GOROOT/bin $CARGO_HOME/bin $CABAL_HOME/bin $HOME/.local/bin $HOME/bin /usr/local/bin /usr/bin /bin /sbin
