function perfflame
    ...
    if math "1 <= "$_flag_p > /dev/null ^ /dev/null
        ...
        set -l clock \e\[1m\e\[92m(string repeat -n $centercol2 " ")
        echo $msg
        echo
        # need time to resume execution, hacky
        time sw $clock perf record -F 99 -g -p $_flag_p -o $df ^ /dev/null
        sleep 0.1 # weird
        perf script -i $df | stackcollapse-perf.pl | grep -v cpu_idle | flamegraph.pl > $ds
        osc52browser $ds
        tput rmcup
        popd
    end
