...
-- TEMP ------------------------------------------------------------ temp ----------------------------------------------------------------------------
	...
	local ntotal_received, ntotal_trans, nmonth_received, nmonth_trans, ntoday_rec, ntoday_trans;
	-- print(stats);
	if(stats ~= "") then
		_,nex,ntotal_received = string.find(stats, "rx:%s*(.-)iB");
		if(ntotal_received ~= nil ) then
			total_received = ntotal_received;
		end
		-- print("tr: "..total_received.."iB");
		...
		if(nmonth_received ~= nil) then
			month_received = nmonth_received;
		end
		-- print("mr: "..month_received..'iB');
		...
		if not today_trans then
			today_trans = 'nil '
		end
		if not month_trans then
			month_trans = 'nil '
		end
		if not total_trans then
			total_trans = 'nil '
		end
		...
		if not today_rec then
			today_rec = 'nil '
		end
		if not month_received then
			month_received = 'nil '
		end
		if not total_received then
			total_received = 'nil '
		end
		...
		cairo_show_text(cr,month_received.."iB");
		cairo_move_to(cr,item_endx + width/15 ,item_endy+item_font_size*5+5);
		cairo_show_text(cr,total_received.."iB");
	end
