...
(defun +amos/workspace-delete ()
  (interactive)
  (let (best-frame best-time time frame)
    (dolist (window (window-list-1 nil 'nomini 't))
      (setq time (window-use-time window))
      (setq frame (window-frame window))
      (setq frame (or (frame-parent frame) frame))
      (when (and (not (eq frame (selected-root-frame)))
...
(defun +amos-workspace-switch-to-frame (frame)
  (let ((oframe (selected-root-frame)))
    (select-frame frame)
    (raise-frame frame)
    (+amos/reset-cursor)
    (recenter)))
...
(defun +amos-workspace-cycle (off)
  (let* ((frames (tty-root-frame-list))
         (n (length frames))
         (index (-elem-index (selected-root-frame) frames))
         (i (% (+ off index n) n)))
