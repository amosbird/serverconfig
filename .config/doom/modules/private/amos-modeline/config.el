...
(defun amos-modeline (key)
  "Returns a mode-line configuration associated with KEY (a symbol). Throws an
error if it doesn't exist."
  (let ((fn (intern (format "doom-modeline-format--%s" key))))
    (when (functionp fn)
      `(:eval (if (minibuffer-window-active-p (selected-window))
                  (with-current-buffer (window-buffer (previous-window)) (,fn))
                (,fn))))))

(defun amos-set-modeline (key &optional default)
  "Set the modeline format. Does nothing if the modeline KEY doesn't exist. If
DEFAULT is non-nil, set the default mode-line for all buffers."
  (when-let* ((modeline (amos-modeline key)))
    (setf (if default
  ...
  (unless after-init-time
    ...
    (dolist (bname '("*scratch*" "*Messages*"))
      (with-current-buffer bname
        (amos-set-modeline 'main)))))

(defun +doom-modeline|set-special-modeline ()
  (amos-set-modeline 'special))

(defun +doom-modeline|set-media-modeline ()
  (amos-set-modeline 'media))

(defun +doom-modeline|set-project-modeline ()
  (amos-set-modeline 'project))
